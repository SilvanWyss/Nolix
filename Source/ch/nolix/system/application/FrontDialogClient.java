...
public final class FrontDialogClient extends Client<FrontDialogClient> {
	 ...
	 */
	public FrontDialogClient(final Application<DialogClient> targetApplication) {
		...
		super(
			...
			c -> {
				c.setDialog(
					new Frame()
					.setController(new FrontDialogClientController(c))
				);
			}
		);
	}
	...
	public void run(final Statement runMethodCommand) {
		internal_getRefDuplexController().appendCommand(DialogClient.RESET_DIALOG_COMMAND + "(" + dialog.getAttributes() + ")");
		internal_getRefDuplexController().appendCommand(INVOKE_RUN_METHOD_COMMAND + "(" + runMethodCommand + ")");
		internal_getRefDuplexController().appendCommand(DialogClient.RESET_OTHER_SIDE_DIALOG_COMMAND);
	...
	protected void internal_run(final Statement command) {
		...
		switch (command.getHeader()) {
			case DialogClient.RESET_DIALOG_COMMAND:
				resetDialog(command.getRefAttributes());
				break;
			case DialogClient.RESET_OTHER_SIDE_DIALOG_COMMAND:
