 ...
 * When a neuron fires, it can calculate and update its output and let other neurons fire.
 * 
 * @author Silvan Wyss
 * @month 2016-11
 * @lines 290
...
public abstract class Neuronoid<N extends Neuronoid<N, I, O>, I, O> {
	...
	private final List<OutputConnection<O>> outputConnections = new List<OutputConnection<O>>();
	 ...
	 */
	public final N addInputNeuron(final Connection<?, ?, I> inputNeuron) {
		return addInputConnection(new InputConnection<I>(inputNeuron.getConnectedNeuron()));
	}
	 ...
	 * @throws RuntimeException if this neuron contains already the given input neuron.
	 */
	public final N addInputNeuron(final double weight, final Connection<?, ?, I> inputNeuron) {
		return addInputConnection(new InputConnection<I>(weight, inputNeuron.getConnectedNeuron()));
	}
	...
	public final AccessorContainer<I> getRefInputs() {
		return new AccessorContainer<I>(inputConnections.to(ic -> ic.getRefOutput()));
	}
	
	//method
	/**
	 * @return the one input of this neuron.
	 * @throws EmptyStateException if this neuron has no input.
	 * @throws InvalidStateException if this neuron has several inputs.
	 */
	public final I getRefOneInput() {
		return getRefOneInputConnection().getRefOutput();
	}
	
	//method
	/**
	 * @return the one input connection of this neuron.
	 * @throws EmptyStateException if this neuron contains no input connection.
	...
	//method
	/**
	 * @return the output connections of this neuron.
	 */
	public final IContainer<OutputConnection<O>> getRefOutputConnections() {
	...
	private final N addInputConnection(final InputConnection<I> inputConnection) {
		...
		inputConnection.getRefInputNeuron().outputConnections.addAtEnd(new OutputConnection<I>(this));
	...
	//method
	/**
	 * Lets this neuron fire and update the given next neurons and visited neurons
	 * if the given visited neurons does not contain this neuron.
	 * 
	 * @param nextNeurons
	 * @param visitedNeurons
	 */
	private void fire(
		final List<Neuronoid<?, ?, ?>> nextNeurons,
		...
		if (!visitedNeurons.contains(this)) {
			...
			final List<Neuronoid<?, ?, ?>> directNextNeurons
			= getRefOutputConnections()
			.getRefSelected(oc -> !visitedNeurons.contains(oc.getRefOutputNeuron()))
			.to(oc -> oc.getRefOutputNeuron());
			
			nextNeurons.addAtEnd(directNextNeurons);
		}
	}
}
